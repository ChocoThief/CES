...
export const eurekaCompanies: Company[] = [
  ...
  },
  {
    ...
    product: '팜픽',
    description: '1. 정의 Pharmpick은 AI 기반 개인 맞춤형 디지털 헬스케어 플랫폼으로, 건강기능식품을 쉽고 안전하게 선택·관리할 수 있도록 돕는 서비스입니다. 2. 핵심 기능 2-1) 성분 분석: 제품에 포함된 기능성 원료를 쉽게 이해할 수 있도록 데이터 기반 분석 제공 2-2) 성분 비교: 유사 제품 간 성분·효능 차이를 한눈에 확인 2-3) 함량 합산: 여러 제품을 함께 섭취할 때 상한치 초과 여부를 자동 계산 2-4) 개발 중 기능 2-4-1) 개인화 추천 알고리즘: 복용 이력·의약품 상호작용을 고려한 안전 맞춤형 추천 2-4-2) 케어링: 가족/지인과 함께 섭취 관리 2-4-3) 셀러브리티: 전문가·셀러가 직접 제품을 제안하고 소비자가 구매까지 이어질 수 있는 샵인샵 3. 비즈니스 모델 3-1) 앱 기반 Freemium 구독 모델 (기본 기능 무료, 고급 기능 유료) 3-2) PB 브랜드 ‘My Name Is’ 제품 판매 연계 → 원스톱 구조 완성 3-3) 데이터 협업: 보험사·리테일러와 연계한 B2B 광고 및 커머스 4. 비전 단순히 건강기능식품 선택을 돕는 것을 넘어, 예방 중심 디지털 헬스케어 플랫폼으로 진화하여 글로벌 시장(특히 의료비가 높은 미국)에서 건강관리 토탈 앱 서비스로 자리 잡는 것.'
  },
  {
    id: 'EP003',
    hall: 'eureka',
    ...
    description: 'Greneta Optimizer AI-Quantization Compressor는 AI 기반 알고리즘을 통해 방대한 3D 모델을 단 몇 초 만에 최대 99.6%까지 축소하면서도 완전한 시각적 품질을 그대로 유지하는 방식으로 3D 콘텐츠의 미래를 변화시킵니다. 이 획기적인 기술은 AR/VR, 디지털 트윈, 의료 영상에서 가장 큰 병목현상 중 하나였던 고정밀 데이터의 무거운 용량 문제를 제거합니다. 깔끔하고 직관적인 인터페이스와 원클릭 프리셋을 통해 복잡한 압축 작업도 손쉽고 전문가 수준으로 수행할 수 있게 됩니다. 모듈형 디자인 덕분에 API 통합 또는 독립 실행형으로도 사용이 가능하여 다양한 플랫폼에서 일관된 결과를 제공합니다. 수 기가바이트(GB)에 달하는 모델도 모바일 XR 기기에서 실시간 스트리밍이 가능할 만큼 가벼워지고, 원격 의료 진단이나 현장에서 즉시 생성되는 디지털 트윈에도 활용될 수 있습니다. 스토리지, 지연 시간, 에너지 비용을 극적으로 줄임으로써, Optimizer는 단순히 작업 흐름을 가속화할 뿐 아니라, 데이터 관련 탄소 배출까지 줄여 지속 가능성에도 기여합니다. 기술적 완성도, 세련된 사용성, 글로벌 확장성을 결합한 Optimizer는 지속 가능한 3D 미래를 형성하는 차세대 혁신 기술로 자리매김하고 있습니다.'
  },
  {
    id: 'EP004',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'AI',
    product: '매니패스트',
    description: '매니패스트는 IT 프로젝트 기획을 가속화하는 AI 에이전트 솔루션입니다. 사용자 요구사항을 단 몇 초 만에 초기 제품 기획안으로 변환하고, 원활한 팀 협업을 위한 기획 전용 작업 공간을 제공합니다. 또한, AI 에이전트와 함께 작업하며 팀은 기획 프로세스의 속도를 획기적으로 높일 수 있습니다. 더 나아가, 매니패스트는 스토리보드나 기능 명세서와 같은 핵심 산출물을 자동으로 생성하여 번거로운 수작업을 없애줍니다. AI가 이미 소프트웨어 개발 분야를 혁신했지만, 매니패스트는 이제 기획 분야에 바로 그 생산성의 혁명을 가져오고 있습니다.'
  },
  {
    id: 'EP008',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'AI',
    product: '라텔',
    description: '디지털 공론장은 제 역할을 잃어가고 있습니다. 여론 조사는 조작에 취약하고, 중요한 논의는 끝없는 디지털 소음 속에 사라집니다. 신뢰할 수 있는 집단적 의사결정 플랫폼이 부재한 상황입니다. Ratel은 이 문제를 해결합니다. Ratel은 자주적 신원(SSI), DAO 거버넌스, AI 기반 도구를 통합해 신뢰할 수 있는 사회적 플랫폼을 구축합니다. DID를 통한 검증된 참여, DAO 기반의 구조화된 의사결정, 그리고 MCP와 같은 AI 도구를 통해 혼란스러운 온라인 공간을 생산적이고 합의 가능한 장으로 바꿉니다. 그 결과, 정책 제안부터 지역사회 문제 해결까지, Ratel은 수동적인 대중을 능동적인 이해관계자로 전환시킵니다.'
  },
  {
    id: 'EP010',
    hall: 'eureka',
  ...
  },
  {
    ...
    product: '랙시안',
    description: 'RAXIAN은 다양한 모빌리티 환경에 최적화된 완전 자율주행 전기 15인승 셔틀입니다. 주식회사 소네트가 독자개발한 AutoDrive™ 레벨 4 자율주행 소프트웨어(한국 국토교통부 인증)를 탑재하여 공공 도로에서 안전한 무인 주행을 가능하게 하며, 인력 수요를 줄이고 접근성을 향상시킵니다. 듀얼 100kW 인휠 모터와 고용량 배터리를 장착해 최고 속도 200km/h, 주행거리 200km를 제공하지만, 출시 모델은 한국 자율주행 규제에 따라 40km/h로 제한될 예정입니다. 모듈형 설계를 통해 크기, 파워트레인, 실내 디자인을 유연하게 재구성할 수 있도록 하여 도심, 농촌, 산업 현장 등 다양한 환경에서 활용될 수 있습니다. 강철 프레임으로 보강한 경량 FRP 모노코그 차체를 통해 제조 비용 절감과 안전성 모두를 달성하였습니다. 산업 디자인 전문기업 INNODESIGN과 협업을 통해 RAXIAN은 독창적인 외관뿐만 아니라, 사용자 중심의 편안하고 접근성 높은 실내 공간까지 갖추었습니다. 또한, 운전석을 제거하여 승객 공간을 극대화하였습니다. 뛰어난 성능, 확장성, 안전한 자율주행 성능을 모두 갖춘 RAXIAN은 차세대 자율주행 셔틀 시장을 선도할 것입니다.'
  },
  {
    id: 'EP012',
    hall: 'eureka',
  ...
  },
  {
    ...
    product: 'Smart RIng',
    description: 'VR/AR 컨트롤러, 에어 마우스, 리모컨을 손가락에 반지 모양의 장치를 착용한 상태에서 작동하는 제품 블루투스를 통해 PC에 연결되며 2시간 동안 완충 후 약 5일간 사용 가격은 약 $100이며 9축 자이로 센서, 1000dpi 광학 센서, 5.0 블루투스, 리튬 이온 50mAh 배터리 등의 부품이 사용'
  },
  {
    id: 'EP014',
    hall: 'eureka',
  ...
  },
  {
    ...
    description: '[제품1] QDLED(퀀텀닷 LED)스마트 바이오 스탠드조명 본 제품은 QDLED기술을 적용한 첨단 스마트 바이오 스탠드 조명으로, **자연광에 가까운 고연색성(High CRI)을 구현하여 사물의 색을 정확하고 생생하게 재현합니다. 이를 통해 사용자는 일반 조명 대비 향상된 시각적 편안함과 선명한 색 표현력을 경험할 수 있습니다. 또한, 본 조명은 근적외선(NIR, Near-Infrared) 방출 기능을 탑재하여 눈의 피로를 완화하고, 장시간 사용 시에도 쾌적한 조명 환경을 제공합니다. 아울러, 지능형 음성인식 제어 기능 및 무선연결을 통해 사용자가 손을 사용하지 않고도 조명의 밝기 및 색온도를 간편하게 조절할 수 있어, 스마트 홈 및 스마트 오피스 환경에 최적화되어 있습니다. [제품2] 5G NR 1W 디지털 ICS MIMO 중계기 5G NR 1W 디지털 ICS MIMO 중계기는 차세대 네트워크 솔루션으로, 실내 공간 어디서든 안정적이고 빠른 5G 연결을 제공합니다. 최대 500 MHz의 초광대역과 MIMO 기술, 그리고 첨단 디지털 간섭 제거 기능을 통해 복잡한 환경에서도 끊김 없는 통신 품질을 보장합니다. 또한 QDLED 시스템과의 연동을 통해, 조명은 단순한 빛을 넘어 지능적이고 데이터 중심의 AI 플랫폼으로 업그레이드 됩니다. 이를 통해 대규모 스마트 웰니스 기기 네트워크에서도 실시간 반응성과 손쉬운 확장성을 구현할 수 있습니다.'
  },
  {
    id: 'EP019',
    hall: 'eureka',
  ...
  },
  {
    ...
    description: '- MRI용 고유전율 웨어러블 패드 : 고자기장 MRI에서 발생하는 영상 왜곡을 독자적인 유전체 패드로 보정하여 SNR, 균일도, 대비를 대폭 향상시키는 진단 솔루션 제공 - AI기반 의료영상 분석 플랫폼 : 클라우드 기반 AI와 LLM 기술을 바탕으로 의료영상 분석, 스포츠 MRI, 병원 PACS 연동 등 다양한 디지털 헬스 솔루션을 제공'
  },
  {
    id: 'EP030',
    hall: 'eureka',
    ...
    description: '아기 분유가 너무 뜨거우면 입을 데일 수 있고, 너무 차가우면 소화에 문제가 생깁니다. 그러나 부모들은 특히 외출 시 분유를 올바른 온도로 준비하는 데 큰 어려움을 겪습니다. 올컴 스마트 텀블러는 펠티어 반도체 기술을 활용해 13℃에서 45℃까지 정밀한 냉·온 제어를 구현함으로써 이러한 문제를 해결합니다. 내부 온도 센서와 블루투스 연동을 통해 모바일 앱에서 실시간 제어가 가능하며, 분리형 SUS-316(수술용 스테인리스 316) 용기는 의료기기와 식품 기구에 널리 사용되는 소재로 위생적 세척과 내구성을 보장합니다. BPA-Free(비스페놀-A 불검출) 외장은 인체에 안전하며, 진공 단열 구조는 보온·보냉 성능을 강화합니다. 모듈형 설계는 유지보수를 간소화하고 제품 수명을 연장합니다. 분유 조제뿐만 아니라 야외 활동, 전문 음료 서비스 등 다양한 환경에서도 최적의 음료 상태를 유지할 수 있어, 정밀 제어·위생·휴대성을 모두 갖춘 생활 혁신 솔루션입니다.'
  },
  {
    id: 'EP031',
    hall: 'eureka',
  ...
  },
  {
    ...
    product: 'UC60',
    description: 'UC60은 60GHz 기반의 초 근거리(5cm 이내) 초고속 저전력 무선 데이터 전송용 반도체 칩으로, USB, UART, SPI, I2C, GPIO, MIPI, Gigabit Ethernet 등 다양한 유선 인터페이스를 무선화할 수 있음. 기존 유선 연결 방식의 단점을 극복하면서도, 최대 6.25Gbps에 달하는 전송 속도를 제공하여 산업 및 소비자용 애플리케이션에서 유선 수준의 신호품질 구현함.'
  },
  {
    id: 'EP034',
    hall: 'eureka',
    ...
    category: 'Digital Health',
    product: '유로린스 라이트',
    description: 'UroRinse™ Light는 장기 요도 카테터 환자를 위해 방광 세척을 자동화하는 의료기기로, 카테터 사용과 관련된 가장 흔하고 치료가 까다로운 병원 내 감염 문제를 직접적으로 해결합니다. 카테터 관련 요로 감염은 오염과 항생제 내성으로 인해 치료가 점점 더 어려워지고 있으며, 기존의 수동 세척 방식은 일관성이 없고 임상 환경에 한정된다는 한계가 있습니다. UroRinse™ Light는 이러한 문제를 대체하여, 버튼 하나로 작동하는 완전 자동화 시스템을 통해 안전하고 표준화된 세척 주기를 안정적으로 제공합니다. 또한 막힘, 혈전, 혈뇨와 같은 합병증을 관리하여 예측 가능하고 예방적인 치료를 가능하게 합니다. 이 제품은 병원, 요양 시설, 가정에서도 활용할 수 있으며, 전문 임상 교육이 없어도 보호자나 환자 스스로 쉽게 조작할 수 있도록 설계되었습니다. 통합 모바일 앱과 보안 클라우드 연결 기능을 통해 원격 모니터링 및 전문가 지원이 가능하며, 의료진은 진료실 밖에서도 환자 관리 할 수 있습니다. UroRinse™ Light는 방광 관리 패러다임을 기존의 사후 치료에서 예방 중심의 선제적 관리로 전환시켜, 필요할 때 어디서나 병원 수준의 치료를 제공합니다.'
  },
  {
    id: 'EP035',
    hall: 'eureka',
    ...
    category: 'Sustainability & Energy',
    product: '유엔지 워터 인핸서',
    description: '물의 흐름만으로 작동하는 세계 최초의 무필터 샤워기용 정수기, UNG 워터 인핸서를 소개합니다. 전기나 배터리 없이, 오직 수압만으로 작동하며, 정기적인 교체나 관리가 필요 없는 친환경 제품입니다. 이 제품은 특허받은 정전기장 모듈을 통해 물 분자 간 수소 결합을 약화시켜, 물의 용해도를 높이고 표면장력을 낮춰줍니다. 이 과정을 통해 물의 경도는 약 25% 감소, 박테리아는 약 66.6% 제거되어 세정력이 향상되고 피부 건강도 보호할 수 있습니다. 또한, 물을 이온화하여 배관 내부의 녹과 스케일 생성을 억제하며, 장기적으로 배관 수명을 연장해줍니다. 정밀 설계된 전극 구조는 베르누이 효과를 극대화하여, 물의 흐름은 빨라지고 사용량은 줄어드는 효과를 제공합니다. 작동 중에는 LED 불빛이 켜져 상태를 쉽게 확인할 수 있으며, 필터 교체나 전기 연결이 필요 없어 유지보수가 전혀 필요 없습니다. 튼튼하고 부식에 강한 재질로 반영구적으로 사용할 수 있고, 일반 샤워 수전부에 간편하게 연결할 수 있어 누구나 손쉽게 설치할 수 있습니다. 이 제품은 피부의 보습력을 높이고, 두피 자극을 줄이며, 피부 건강 전반을 개선하는 데 도움을 줍니다. UNG 워터 인핸서는 물의 질을 한층 업그레이드하여, 건강하고 지속 가능한 샤워 환경을 제공합니다.'
  },
  {
    id: 'EP036',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'Vehicle Tech & Advanced Mobility',
    product: '인투커넥스',
    description: 'int2ConneX는 한국에서 독자적으로 개발된 DDS(Data Distribution Service) 표준 기반의 실시간 데이터 통신 미들웨어로, 고신뢰성과 미션 크리티컬 애플리케이션을 위해 설계되었습니다. 서버리스(peer-to-peer) 메시 아키텍처와 토픽 기반 Publish/Subscribe 모델을 결합하여 분산 시스템 환경에서 저지연·결정론적 통신을 구현합니다. 모듈형 런타임은 EtherCAT, CAN-FD와 같은 산업용 프로토콜을 지원하며, Real-Time Linux, RTOS, 컨테이너 기반 플랫폼에 효율적으로 배포할 수 있습니다. 또한 Adaptive AUTOSAR와 완벽히 호환되어 Some/IP 통신 모듈이나 차세대 자동차 플랫폼을 대체할 수 있습니다. 중앙 서버를 제거함으로써 엣지 컴퓨팅 성능을 최적화하고, 내결함성(Fault Tolerance)과 확장성을 향상시켰습니다. 특히 자율주행차, 스마트 제조, 국방, 로보틱스 분야에서 혹독한 환경에서도 무손실·고신뢰 데이터 교환을 보장합니다. 국내외 특허는 DDS 기반 통신 및 전송 통합 기술을 보호하고 있으며, 상용 버전과 연구용 버전은 엣지 노드 단위의 유연한 라이선스 정책을 제공해 기업 시스템에서 폭넓게 활용될 수 있습니다.'
  },
  {
    id: 'EP038',
    hall: 'eureka',
    ...
    description: 'Gen AMS는 단순한 모니터링을 넘어, AI 기반의 스마트 관제 시스템입니다. AI가 실시간으로 상황을 분석하고 위험을 예측하여 그 결과를 자동 보고서 형태로 제공합니다. 기존 인력에 의존하던 육안 관제를 AI 관제로 전환하여 관제의 효율성을 극대화합니다. 또한, 여러 목적에 맞는 다목적 동시 관제 기능을 지원하여 필요한 탐지 목적에 따라 맞춤형 모니터링이 가능합니다. 안전 데이터를 생성하고 사고 원인을 분석해 통계를 제공함으로써 데이터 기반의 안전 관제를 통해 사고를 선제적으로 예방하도록 돕습니다. 언제 어디서든 즉각적인 대응이 가능하도록 24시간 모바일 지원을 통해 스마트폰으로 사고 상황을 실시간으로 보고받을 수 있습니다.'
  },
  {
    id: 'EP039',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'AI',
    product: '직퍼',
    description: '⦾ AI 음악성분조합 시스템을 탑재한 저작권통합 콘텐츠플랫폼 『직퍼』 - 음악사용자(영상, 게임, 메타버스, 방송)와 음악생산자(음악가, 기획사)에게 특화된 콘텐츠플랫폼 - AI 음악성분조합 검색·추천 시스템: 콘텐츠 제작자에게 필요한 음악을 객관적인 음악특성으로 정의하여 검색 - 검색한 고퀄리티의 발매음악을 저작권걱정없이 자유롭게 사용 - 음반사·유통사 기능 내장: 중간과정없이 아티스트 계약·음악발매·정산'
  },
  {
    id: 'EP042',
    hall: 'eureka',
  ...
  },
  {
    ...
    description: '(스카이빙 방열판) 스카이빙 공정은 재료를 정밀하게 얇게 절단하여 표면적을 획기적으로 늘리고 최적의 방열을 구현하는 차세대 방열 기술입니다. 탁월한 열전도성과 가공성, 생산성을 제공하며, 기존 방열판 대비 15~20% 높은 방열 효율을 달성합니다. (스카이빙 CNC 설비) 스카이빙을 방열판을 제조하는 설비로 기존 설비보다 1/2사이즈로 작지만, 기존 스카이빙 설비보다 빠르고 정교한 작업을 수행할 수 있어 높은 생산 능력을 자랑합니다. 회전형 베드는 정교한 스카이빙 작업이 가능하여 다양하고 복잡한 형태의 특수 형상 핀을 제작 할 수 있습니다. 4면의 진공베드를 모두 활용하기 때문에, 빠른 교체가 가능하고 다양한 형상의 제품을 동시에 가공할 수 있습니다.'
  },
  {
    id: 'EP045',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'Digital Health',
    product: '에이모스트',
    description: 'Aimost는 세계 최초로 스마트폰만으로 호흡기와 심장의 이상 소리를 정확하게 잡아내는 AI 소프트웨어입니다. 이 솔루션은 실제 병원에서 소아부터 성인 환자까지 수집된 25만 건 이상의 청진 데이터를 바탕으로 만들어졌습니다. 전 세계에서 가장 크고 다양한 데이터셋을 사용했기 때문에, 중요한 호흡기·심장 질환을 믿을 수 있는 정확도로 찾아낼 수 있습니다. 최신 트랜스포머 기반 AI 모델을 적용해 빠르고 효율적인 성능을 제공하며, 별도의 청진기나 웨어러블 기기 같은 장비가 전혀 필요하지 않습니다. 스마트폰 하나로도 crackle, wheezing 같은 호흡기 질환 관련 이상음은 물론, 심장 잡음(murmur)이나 부정맥(arrhythmia)까지 탐지할 수 있습니다. 현재 임상시험이 진행 중이며, 2025년에는 식품의약품안전처(MFDS) 승인을 목표로 하고 있습니다. 이어서 미국 FDA와 유럽 CE 인증도 준비 중입니다.'
  },
  {
    id: 'EP049',
    hall: 'eureka',
  ...
  },
  {
    ...
    description: '라이트브릿지㈜의 **PEM 수전해 시스템(LBEX 시리즈)**은 재생에너지를 활용해 고순도(99.999%)의 그린수소를 생산하는 고효율 수소 생산 설비입니다. 고전류밀도 운전과 빠른 기동성을 구현하여 태양광·풍력 등 변동성 전원과의 연계에 최적화되어 있으며, 모듈형 설계를 통해 5kW에서 MW급까지 확장 가능합니다. 또한 자체 개발한 H-Bridge 통합형 수소충전 플랫폼을 통해 생산–저장–압축–충전이 가능한 분산형 수소 인프라 솔루션을 제공합니다. 본 시스템은 CE·PED·ISO 등 국제 인증 기준을 충족하며, 탄소중립 실현과 지속가능한 에너지 전환을 위한 차세대 그린수소 기술로 주목받고 있습니다.'
  },
  {
    id: 'EP051',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'AI',
    product: '모플 프레드박스',
    description: '<제품 개요> PredBox는 AI 기반 수요 예측과 업무 자동화를 통해 재고 관리를 최적화하는 구독형 B2B SaaS 서비스입니다. 모플의 솔루션은 소비재 기업의 모든 재고 관련 문제(품절, 결품, 과재고 등)를 예방하며, 공급망 전반에서의 비효율을 제거하여 비즈니스 성과를 제공합니다. <핵심 기능> 1. 품절과 과재고 문제 해결 PredBox는 수 십억 건에 달하는 대규모 재고 트랜잭션 데이터를 지속적으로 분석하고 모니터링 합니다. 이를 통해 물류 프로세스 내 병목 현상을 감지하고, 잠재적인 손실이 발생하기 전에 예측하여 품절과 과재고 상황을 예방합니다. 기업은 데이터를 기반으로 선제적으로 재고를 관리할 수 있습니다. 2. 독자적인 AI 수요예측 기술 PredBox의 핵심에는 인간 수준의 전문성과 대규모 데이터 학습을 결합한 모플 자체 개발 AI 엔진이 있습니다. 이 독자적인 모델은 빠르게 변화하는 트렌드나 데이터가 부족한 소수의 핵심 상품의 수요도 정확하게 예측합니다. 또한, 별도의 하드웨어 추가 설치 없이, 거래 데이터를 활용한 비카메라 방식의 ‘선반 재고 확인’ 기능을 제공하여 효율적인 운영을 지원합니다. 3. 진정한 엔드투엔드 자동화 PredBox는 재고관리 전 과정에서 발생하는 물류 제약 요소를 찾아내고, 이를 해결하여 완전한 엔드투엔드 업무 자동화를 실현합니다. 수요 예측, 재고 분배, 보충 계획을 하나의 통합된 시스템에서 관리함으로써, 결품 및 과재고가 완전히 사라질 때까지 지속적인 최적화를 수행합니다. <성과 및 영향> PredBox는 국내 파트너사와의 실제 운영에서 성과를 입증했습니다. 대한민국의 대표 패션 브랜드 신성통상은 모플 PredBox의 AI 기반 수요 예측 및 업무 자동화 구독 서비스를 도입해, 재고 관리 방식을 혁신 했습니다. - 매출 6천만 달러(한화 800억원) 증가 - 기획팀 기준 업무 시간 96% 단축 - 결품률 56% 감소 이러한 결과는 AI 기반 자동화가 실제 비즈니스 성과에 직접적으로 기여함을 보여줍니다. 모플의 PredBox는 예측 결과를 실제 수익 및 운영 효율화로 연결하는 역량을 가진 솔루션 입니다.'
  },
  {
    id: 'EP056',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'AI',
    product: '심투드론',
    description: '드론 센서데이터를 실시간 가상환경에 반영해 AI 탐지·경로 최적화·관제를 지원하는 플랫폼입니다. 국방, 재난, 산업 현장에서 실시간 시뮬레이션과 효율적 드론 운용을 가능하게 합니다.'
  },
  {
    id: 'EP060',
    hall: 'eureka',
  ...
  },
  {
    ...
    description: '1. 구강세정기(하드웨어) - 물탱크 없는 직수 연결, 버튼 하나로 간편 사용 - 솔레노이드 제어로 최적 수압 및 3가지 맥동 모드 제공 - 150g 초경량 분리형 그립, 교체 가능한 위생 호스 및 불소 필터 - 최소 유지관리, 물만으로 사용 가능, 글로벌 확산 가능 - 미니멀하고 세련된 디자인, 욕실 인테리어와 조화 - 단순한 구강기기를 넘어 웰니스 오브제로 기능 2. AI 기반 구강관리 솔루션 - 파노라마 영상을 통한 구강 분석 및 점수화 - 맞춤형 관리 가이드, 트렌드 추적, 구강 건강 챌린지 제공 - RAG 기반 챗봇으로 개인 맞춤형 상담 및 행동 유도'
  },
  {
    id: 'EP062',
    hall: 'eureka',
    ...
    category: 'Digital Health',
    product: 'HEXA',
    description: 'HEXA는 흉부 CT 영상을 쉽고 직관적으로 확인할 수 있는 의료영상 소프트웨어입니다. 의료영상 표준 규격인 DICOM을 준수하며, CT 영상의 픽셀 데이터를 기반으로 2차원(축상, 관상, 시상 단면) 및 3차원(볼륨 렌더링) 영상을 제공합니다. 사용자는 Windowing Tool을 통해 관심 있는 HU(Hounsfield Unit) 범위를 설정하여 특정 조직을 강조하거나, Cropping Tool을 이용해 원하는 영역만 선택적으로 관찰할 수 있습니다. CT 영상은 PACS 또는 사용자가 직접 불러온 데이터를 활용하며, 소프트웨어 내에서 자동으로 저장·처리됩니다. HEXA는 의료 전문가가 흉부 CT 영상을 효율적으로 탐색하고 분석을 보조할 수 있도록 설계된 의료영상 전송 소프트웨어입니다.'
  },
  {
    id: 'EP063',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'AI',
    product: '지능형메시지(지메시)',
    description: '1. Cloud기반 SaaS 메시지 2. OnClick 지능형 전송처리 3. OnClick 템플릿 자동생성 -전송료 60% 절감 -도입비용 50% 절감 -노동력 30% 절감 -수신 만족도 70% UP'
  },
  {
    id: 'EP066',
    hall: 'eureka',
  ...
  },
  {
    ...
    description: 'SOC Canvas는 복잡한 시스템온칩(SoC) 개발 과정을 제거한 노코드 설계 솔루션입니다. 저수준 RTL 코드를 직접 작성하는 대신, 엔지니어는 직관적인 시각적 인터페이스를 통해 시스템 다이어그램을 구성합니다. 이 추상화된 설계 도식을 기반으로 RTL, UPF, SDC, DFT와 같은 설계 산출물이 몇 분 만에 자동으로 생성됩니다. 오늘날 SoC는 200억 개가 넘는 트랜지스터를 포함하면서, 기존의 수작업 중심 설계 방식은 한계에 다다르고 있습니다. SOC Canvas는 설계를 시스템 레벨로 끌어올려 제어 로직을 재사용 가능하고 커스텀 가능한 블록으로 추상화함으로써, 엔지니어링 노력을 크게 줄이고 개발 기간을 단축하며 사람에 의한 오류를 최소화합니다. 설계 시간을 단축하고 SoC 개발의 장벽을 낮춤으로써 SOC Canvas는 더 많은 팀이 AI, 자동차, 엣지 디바이스용 맞춤형 칩을 만들 수 있도록 돕습니다. 아이디어에서 실리콘까지의 여정을 가속화함으로써, 전 세계적으로 증가하는 더욱 스마트하고 효율적인 기술 수요를 충족시키는 데 기여합니다.'
  },
  {
    id: 'EP068',
    hall: 'eureka',
  ...
  },
  {
    ...
    category: 'Robotics',
    product: '모티글러브 2',
    description: '로봇 손의 원격 조작 및 학습을 위한 고정밀 핸드 트래킹 글러브입니다. 작년 CES 2025에서 혁신상을 수상한 Motiglove 1의 기술을 기반으로, 새로운 센서를 추가해 트래킹 정확도와 안정성을 한층 더 향상시켰습니다. Motiglove 2는 로보틱스, 휴머노이드, 텔레오퍼레이션 등 다양한 분야에서 인간의 손 움직임을 정밀하게 트래킹하고 학습할 수 있도록 지원합니다.'
  },
  {
    id: 'EP071',
    hall: 'eureka',
    ...
    description: 'TM-MX AEMS 케이블은 액체 증발 상변화 냉각 방식을 활용하여 메가와트급 고속 충전 시 발생하는 과열 및 안전 문제를 해결하는 세계 최초의 경량, 고효율 충전 솔루션입니다. 대형 전기차, 선박, 도심 항공 모빌리티(UAM) 플랫폼 등에서 초고속 충전에 대한 수요가 증가함에 따라, 케이블의 무게와 열 관리는 매우 중요해졌습니다. AI 기반의 자율 에너지 관리 시스템(AEMS)은 온도, 전력 흐름, 냉각 매개변수를 실시간으로 지속해서 모니터링하고 최적화하여, 기존 CCS(Combined Charging System) 인프라에서도 안전하고 효율적인 충전을 가능하게 합니다. 메가와트급으로 5분 이내에 완전 충전이 가능하여, 운영 중단 시간을 줄이고 시스템 신뢰도를 향상합니다. 상변화 냉각 기술은 전도체 성능을 높이는 동시에 케이블 무게를 17.6파운드(약 8kg) 미만으로 크게 줄여, 신속한 설치, 유지보수, 현장 배치를 지원합니다. 이러한 혁신은 첨단 안전 및 호환성 기능과 결합하여 상업, 해양, 산업 부문에서 전기 모빌리티의 광범위한 채택을 가속합니다.'
  },
  {
    id: 'EP072',
    hall: 'eureka',
    ...
    category: 'Vehicle Tech & Advanced Mobility',
    product: '오르비온트',
    description: '지상망 의존으로 끊기던 긴급 통신을 위성 레이어로 보강하고, 생성형·경량 sLLM과 음성→텍스트 코드 전송, 이벤트 기반 웨이크-업 등 자체 기술로 불안정한 통신 환경에서도 구조 확률을 높입니다. 1. Vehicle AECS/e-Call 모듈 & 서비스(B2B): 차량 사고 전·중·후 상황 및 탑승자 상태를 AI가 판단해 긴급구조센터와 자동 커뮤니케이션. 위성 기반 백업으로 지상망 불능 시에도 연결 유지. 재난·산불·홍수 등 전방 위험 지역 위성 분석을 통한 우회 경로 제시. 충돌로 차량 위성 안테나가 손상된 경우에도 영상위성 분석으로 이상 징후 감지 및 센터 알림. 2. Personal SOS 디바이스(B2G/B2C): 2026년 하반기 출시 목표. 동적 메시지 압축(핵심 단어·코드 우선 전송, 대역폭 약 40% 절감), sLLM 기반 자동 응답·상황 요약, 시니어 친화 음성 인식(STT-TTS)과 이벤트 기반 동기화(SOS 버튼/낙상 등) 탑재. Skylo와 협업 진행.'
  },
  {
    id: 'EP073',
    hall: 'eureka',
...
export const globalCompanies: Company[] = [
  ...
  },
  {
    ...
    description: 'PaDO는 자율주행과 실시간 센서 기술을 기반으로 수질을 모니터링하고 녹조를 예방·제거하는 친환경 솔루션입니다. AI 기반 수질 예측 알고리즘을 통해 이상 수질 징후를 조기에 감지하여 선제적인 대응이 가능하며, 화학약품을 사용하지 않아 생태계에 부담을 주지 않습니다. 분수 기능과 LED 조명을 결합하여 수질 개선뿐 아니라 수변 공간의 미관을 향상시키며, 공원·호수·리조트·도심 수변 등 다양한 환경에서 활용할 수 있습니다. PaDO는 지자체, 공공기관, 민간시설을 위한 스마트시티 수질 관리와 ESG 친화적 수변 관리 솔루션으로 자리매김하고 있습니다.'
  },
  {
    id: 'GP003',
    hall: 'global',
  ...
  },
  {
    ...
    description: 'Deepscent Lounge AI-Multisensory(가칭)는 IoT 기반의 취향 맞춤형 디지털 향 기 솔루션입니다. - 향기캡슐과 발향을 위한 하드웨어, 제어 소프트웨어 및 인터페이스(모바일 앱)로 구성됩니다. - 하드웨어에는 4개의 향기 캡슐을 선택해 삽입하고, 개별 캡슐의 발향 세기를 조 절해 원하는 분위기를 연출할 수 있습니다. - AI를 활용하여, 연출하고 싶은 분위기를 이미지, 사운드, 보이스로 입력했을 때 이에 적합한 향기 배합을 자동으로 설정해 공간 경험의 질을 향상시킵니다.'
  },
  {
    id: 'GP008',
    hall: 'global',
  ...
  },
  {
    ...
    category: 'Sustainability & Energy',
    product: 'Artenix™ AI',
    description: 'Artenix™ AI는 AI 반도체 제조용 웨이퍼 레벨 센서에 최적화된 차세대 초박형 배터리입니다. 고진공(1mTorr), 급격한 열 사이클, 강한 전자기 간섭 등 극한 환경에서도 안정적으로 작동하며, 탁월한 표면 평탄도를 통해 포토리소그래피 및 식각 정밀도에 영향을 주지 않습니다.'
  },
  {
    id: 'GP011',
    hall: 'global',
    ...
    category: 'XR',
    product: 'Genpresso',
    description: '아이디어를 이미지, 텍스트, 영상 등으로 입력하고 3D 모델을 생성하는 워크플로우를 AI 에이전트와 함께 하여 효율적으로 제작할 수 있는 플랫폼 다양한 사용자와 함께 협업하여 작업할 수 있고, 여러 AI 모델들을 사용하여 효과적으로 디자인 작업을 할 수 있음'
  },
  {
    id: 'GP012',
    hall: 'global',
  ...
  },
  {
    ...
    description: '■ESS(Energy Storage System )는생산된전력을 저장했다가 필요할때공급하는 시스템으로 전력저장장치,전력변환장치, 제반 운영 시스템 ■전력부족에 대한 대안: 에너지 산업필수요소는, ESS'
  },
  {
    id: 'GP015',
    hall: 'global',
  ...
  },
  {
    ...
    description: 'AI 기반 친환경 수상 드론은 해양 쓰레기 수거, 해조류 제거, 수질 모니터링, 방사능 탐지, 인명 구조 등 다양한 해양 임무를 수행하도록 설계·개발되었습니다. AI 기반의 다중센서(Lidar, Camera, AIS 등) 비전 기술을 통해 해양 객체를 탐지 및 식별하여, 자율운항/충돌회피 알고리즘을 통해 추진제어 기능을 구현합니다. 또한 최첨단 기술과 지능형 설계를 통해 해양 안전, 환경 보호, 그리고 지속 가능한 미래에 기여하며, 주요 임무는 다음과 같습니다. - 해양 쓰레기 수거 - 해조류 제거 - 수질 모니터링 - 방사능 탐지 - 해양 인명 구조 친환경 수상 드론은 혁신성과 지속가능성을 결합한 해양 기술의 중요한 도약을 의미합니다.'
  },
  {
    id: 'GP022',
    hall: 'global',
    ...
    description: '자가발전 중창이 장착된 신발은 사용자가 걸을 때 발생하는 압력과 그 압력에 의한 복원력을 활용하여 전력을 생산합니다. 넘어지거나 무너져 신발 각도가 일정 수준 이상 기울어지면 사용자의 위치와 APP가 보호자에게 전송됩니다. 치매 환자 배회, 납치 및 유괴, 아동 실종 등의 사고 예방에 매우 유용합니다. 일정 시간(24시간) 동안 신발을 착용하지 않으면 보호자에게 알림을 전송합니다.'
  },
  {
    id: 'GP023',
    hall: 'global',
  ...
  },
  {
    ...
    category: 'AI',
    product: '애드아이',
    description: '옥외광고가 정밀성과 측정 가능성을 갖춘 새로운 시대에 진입하고 있습니다. 기존의 옥외광고는 무작위 노출에 의존하여 타겟팅 한계가 뚜렷했지만, 당사 솔루션 애드아이는 이를 근본적으로 혁신합니다. 고도화된 카메라 및 영상 분석 기술을 통해 디지털 사이니지 앞의 실제 관람객을 분석하고, 성별·연령별로 광고를 실시간 매칭함으로써 온라인 플랫폼(예: 유튜브)이 이용자 기반 타겟팅으로 다섯 배 성장을 이룬 것처럼 옥외 매체 또한 정밀 타겟팅을 실현할 수 있습니다. 또한 시선 추적을 통해 실제 광고 주목도를 측정함으로써 신뢰성 높은 성과 데이터를 제공합니다. 이를 바탕으로 광고주는 캠페인을 최적화하고, 콘텐츠를 재설계하며, 광고 전략을 실제 데이터 기반으로 개선할 수 있습니다. 정적인 옥외광고를 적응형·데이터 기반 플랫폼으로 전환함으로써, 오프라인 미디어를 디지털 광고의 정밀성과 연결합니다. 실시간 분석과 동적 송출을 통해 광고 효과를 극대화하고, 관람객과의 적합성을 높이며, 전 세계적으로 새로운 수익 기회를 창출합니다.'
  },
  {
    id: 'GP031',
    hall: 'global',
  ...
  },
  {
    ...
    description: '표면 처리 전문 솔루션, \'Surface Pro 솔루션\'은 로봇 팔 끝에 장착되어 디버링, 샌딩, 폴리싱 등 표면 마감 공정을 자동화하는 EOAT 솔루션입니다. 능동형 컴플라이언스 구조와 6축 힘/토크 센서를 기반으로 정밀 제어 기능을 통합하여, 작업자의 경험과 숙련도에 의존하던 반복 작업을 정량적이고 일관된 높은 품질로 자동화할 수 있습니다.'
  },
  {
    id: 'GP034',
    hall: 'global',
  ...
  },
  {
    ...
    category: 'AI',
    product: '홍채인식솔루션',
    description: '우리는 홍채인식에 적합한 카메라모듈과 IR-LED 자체적으로 개발하여 사용하므로 인식률이 높고 저렴하다. 우리의 홍채인식솔루션은 근거리, 실내용으로만 사용되는 기존 기술을 한계를 넘어 원거리, 안경착용자도 실내 및 그늘진 곳에서 홍채인식이 가능하다. 우리는 본 CES에서 놀라운 1미터에서 안경착용자도 홍채+얼굴인식이 가능한 제품을 선보일것이다. 또한 불가능한 3미터에서 안경착용자도 홍채+얼굴인식이 가능한 제품을 선보일것이다. 1미터, 3미터 홍채인식솔루션은 전세계에 없는 기술입니다. 이 기술이 가능하게된 것은 AI기술과 우리의 광학기술의 앙상불적인 접목 때문입니다.'
  },
  {
    id: 'GP037',
    hall: 'global',
  ...
  },
  {
    ...
    category: 'Accessibility & AgeTech',
    product: '베이스에이아이',
    description: 'Baxe AI는 누구나 사용하는 스마트폰을 시니어와 가족을 위한 비침습적 건강 모니터링 도구로 바꿔줍니다. 단 5초의 카메라 스캔만으로 심박수, 혈압, 산소포화도, 스트레스 지수를 웨어러블이나 병원 방문 없이 확인할 수 있으며, AI는 측정된 생체신호와 감정·행동 데이터를 바탕으로 맞춤형 대화형 가이던스를 제공합니다. 가족들은 이상 징후 발생 시 즉시 알림을 받아 안심할 수 있고, 시니어들은 집에서도 24시간 독립적으로 건강을 관리할 수 있습니다. 요양 시설에서 진행한 파일럿 결과, 간병인의 일일 활력징후 확인 시간이 최대 80% 단축되어 효율성과 관리 품질이 동시에 향상되었습니다. 웹 브라우저만 있으면 바로 사용할 수 있으며, 큰 버튼, 음성 안내, 원터치 실행 등 고령자 친화적 UI를 제공합니다. SK그룹, 소프트뱅크 로보틱스에서 PoC 를 진행, 몬트리올 대학교와 공동 연구 중에 있으며, 200명의 시니어 대상 시험에서 손 떨림이나 저조도 환경에서도 90% 이상 안정적으로 스캔이 이뤄졌습니다. 현재는 FDA 510(k) 인증 절차를 진행 중이며, 임상 수준의 정확도를 목표로 하고 있습니다.'
  },
  {
    id: 'GP039',
    hall: 'global',
    ...
    description: 'N-Intellifex는 NSOFT의 차세대 AI 플랫폼으로, 제조 데이터를 실행 가능한 인텔리전스로 전환합니다. ERP, MES, WMS 및 기타 기업 시스템의 데이터를 활용하여, 전체 생산 파이프라인에 걸쳐 실시간 인사이트와 자동화를 제공합니다. 강력한 동적 AutoML 엔진을 기반으로, N-Intellifex는 물류, 전자, 자동차, 기계 산업 등 각 산업에 맞게 AI 모델을 지속적으로 최적화합니다. 문맥을 인식하는 분석 기술과 산업별 맞춤형 AI를 결합하여, 더 스마트한 의사결정, 더 빠른 문제 해결, 더 정밀한 예측을 가능하게 합니다. 구조화된 데이터베이스, 시각적 데이터, 실시간 센서 스트림 등 다양한 형식의 멀티모달 데이터를 활용하여, N-Intellifex는 복잡함을 명확함으로, 데이터를 가치로 전환하는 진정한 제조 인텔리전스를 실현합니다.'
  },
  {
    id: 'GP040',
    hall: 'global',
  ...
  },
  {
    ...
    description: 'IoT와 스마트 팩토리 시대의 가장 큰 걸림돌은 센서의 \'전원\' 문제였습니다. 복잡한 유선 공사와 주기적인 배터리 교체는 막대한 비용과 관리 부담, 나아가 안전사고의 위험까지 야기했습니다. Harvenergy™는 이 문제의 근원을 제거하는 혁신적인 자가발전 무선 센서 시스템입니다. 고전 물리학 이론인 페러데이 법칙을 적용 동작의 신뢰성을 확보하고, 화재의 열이나 설비의 움직임 같은 이벤트 에너지를 스스로 전력으로 변환하고, 감지된 정보를 즉시 무선으로 송신합니다. 최근 획득한 국제 방폭 인증(IECEx)은 Harvenergy™가 정유, 화학 플랜트 등 최고 위험 등급의 환경에서도 절대적으로 안전함을 증명하며, 안전을 위한 모니터링 영역을 극대화시켰습니다. Harvenergy™는 단순한 부품을 넘어, \'제로-메인터넌스\', \'인스톨-애니웨어\', \'심리스-인테그레이션\'을 통해 가장 진보된 IoT 인프라를 완성하는 궁극의 솔루션입니다.'
  },
  {
    id: 'GP042',
    hall: 'global',
    ...
    category: 'Lifestyle',
    product: '애니파워',
    description: '애니파워(AnyPower) – 차세대 산업·아웃도어 파워뱅크 제품 개요 애니파워는 전문가, 아웃도어 활동가, 재난 대응 인력을 위한 프리미엄 다목적 에너지 솔루션입니다.기존 파워뱅크와 달리 약 40% 가벼우며, 다양한 전동공구와 호환 가능, 그리고 차세대 PD3.1 (240W) 초고속 충전 기술을 탑재했습니다.애니파워는 단순한 배터리를 넘어 생산성을 높이는 에너지 플랫폼으로, 작업 현장·아웃도어 활동·재난 상황에서 새로운 표준을 제시합니다. 1. 주요 특징 1)초경량 설계 ∘기존 산업용 파워뱅크 대비 약 40% 가벼워 장시간 사용 시 피로도를 최소화합니다. 2) 범용 전동공구 호환성 ∘애니파워의 특허받은 어댑터 기술을 통해 다양한 브랜드의 전동공구를 하나의 배터리로 구동할 수 있어, 별도의 충전기나 배터리가 필요하지 않습니다. 3) 멀티 포트 초고속 충전 ∘USB-C PD3.1 (최대 240W), USB-A (QC), 산업용 XT90/GX16 포트를 지원하여 노트북, 드론, 마이크로 모빌리티 등 다양한 기기를 빠르게 충전할 수 있습니다. 4) 안전성과 신뢰성 ∘UN38.3, IEC 62133-2, KC, PSE등 글로벌 인증을 획득했으며, BMS에 AI 기반 모니터링 기술을 적용하여 온도·과전류·작업자 안전 경보까지 제공합니다. 5) 모듈형 & 다목적 활용 ∘100Wh(항공 반입 가능), 200Wh모듈형으로 제공되어, 산업 현장은 물론 해외 출장·여행에도 유연하게 대응할 수 있습니다. 2. 적용 분야 1) 건설·산업 현장– 범용 배터리로 공구 가동 중단 시간을 줄임 2) 아웃도어 & 레저– 캠핑, 하이킹, 원정 활동에 최적의 휴대 전원 3) 재난·비상 상황– 위급 시 안정적인 전원 공급 3) 글로벌 모빌리티– 국제 규격에 맞춘 휴대 전원 솔루션 3. CES 2026 혁신 포인트 애니파워는 배터리를 넘어 스마트 에너지 플랫폼입니다. CES 2026에서는 차세대 애니파워 프로(AnyPower Pro)를 선보입니다: 1) AI 기반 작업자 안전 모니터링 시스템(낙상 감지, 긴급 알림) 2) 스마트 에너지 관리 앱– 실시간 배터리 상태 및 진단 기능 제공 3) 친환경 설계– 재활용 소재 적용으로 ESG 목표 달성 기여'
  },
  {
    id: 'GP043',
    hall: 'global',
    ...
    category: 'AI',
    product: 'AI 재고실사 로봇',
    description: '와따에이아이의 AI 물류 재고관리 로봇은 하이랙과 평치 등 다양한 형태의 창고에서 이뤄지던 수작업 재고 실사를 자동화해 불필요한 인력 투입과 오류 발생을 크게 줄여줍니다. 라벨과 바코드를 읽고, 적재 위치를 확인하며, 여러 물품을 동시에 인식해 보다 정확하고 신속한 재고 관리를 가능하게 합니다. 그 결과 실시간 데이터에 기반한 지속 가능한 창고 디지털트윈을 구축하여 운영 효율성과 생산성을 한 단계 끌어올립니다.'
  },
  {
    id: 'GP044',
    hall: 'global',
  ...
  },
  {
    ...
    category: 'Digital Health',
    product: '지니소리3',
    description: '지니소리3는 스마트 청각 보조 솔루션 전문 스타트업 유위컴㈜이 개발한 특허 기술 기반 무선 송신기와 보청 기능 결합 방식을 적용한 차세대 청력보조기기입니다. 1. 실시간 음성 전달·증폭: 잡음이 많거나 거리가 있는 환경에서도 선명한 청취 가능 2. 무선 연결: 차세대 Bluetooth LE Audio를 적용한 이어폰·헤드셋과 자동 연동, 고령층도 간편하게 사용 3. 다양한 활용 환경: 개인 난청 보조, 상담 창구, 가정 방문 서비스 등에서도 효과적 4. 맞춤형 난청 보조: 경도~중고도 난청을 지원하는 7가지 보청 모드 탑재 현재 지니소리 시리즈는 전국 500여 개 공공기관·병원에서 상담과 소통 지원에 활용되고 있으며, 지니소리3는 이를 업그레이드한 프리미엄 청력 지원 솔루션입니다.'
  },
  {
    id: 'GP048',
    hall: 'global',
    ...
    category: 'Mobile Devices, Accessories Apps',
    product: 'Safe-T3',
    description: '1. Safe-T3는 운전자 안전과 친환경 운행을 동시에 실현하는 AI 기반 차량관제 플랫폼입니다. 실시간 위치 추적, 경로 재생, 공회전 및 충격 감지 등의 기본 기능을 제공합니다. 2. 특히 운전대에 내장된 생체신호 센서를 통해 심박수와 스트레스 지수를 수집하고, AI가 이를 분석해 졸음, 저혈당, 미주신경성 실신 등의 징후를 조기에 감지함으로써 사고를 사전에 예방합니다.'
  },
  {
    id: 'GP049',
    hall: 'global',
  ...
  },
  {
    ...
    description: '온도센서.가스센서 및 기존 건물 화재관리 시스템과 연동된 데이타를 통해 위험구역과 화재상황을 휴대폰으로 전송하여 위험상황을 감지합니다. 또한, 설치된 디바이스 에서는 해당 데이터를 기반으로 화재현장의 위험구역과 안전구역을 블록단위로 설정하여. 대피자가 능동적으로, 화재현장 주변소음의 주파수를 회피한 부저소리와, 고휘도의 밝은 LED 를 통한 안전구역 으로의 탈출로를 안내받을수 있습니다. * 특징 - 화재현장 부저알림음, 고휘도 LED : 현장의 소음을 마이크로 수음하여, 현장의 소음 데시벨 및 주파수를 회피한 부저음 을 방사하고. 고휘도의 밝은 LED 를 송출하여.장애인.노령층 등의 안전확보 시스템 제공.'
  },
  {
    id: 'GP051',
    hall: 'global',
  ...
  },
  {
    ...
    description: '인스파워의 스마트 브로드밴드 커넥티비티 증폭기는 차세대 5G, IoT, XR 환경에서 안정적이고 넓은 무선 커버리지를 제공하는 완제품 시스템입니다. 이 장비는 고출력 RF 증폭 기술과 더불어 이더넷 기반의 지능형 제어 기능, 터치 LCD 인터페이스를 갖추어 사용자가 쉽게 설치하고 운용할 수 있습니다. 스마트시티, 연구소, 기업 및 프로슈머 환경에서 무선 연결 성능을 확장하는 핵심 솔루션입니다.'
  },
  {
    id: 'GP054',
    hall: 'global',
    ...
    description: 'SACR-GuardMate는 공사 현장의 안전을 혁신적으로 강화하기 위해 개발된 안전 보조 캐리어 로봇입니다. 센서와 카메라로 작업자의 생체 신호와 행동을 실시간 분석하고, 보통·경고·위험 단계로 분류합니다. 분류된 단계에 따라 경고·위험 단계는 선제적 대응을 가능하게 합니다. 맨홀·탱크 등 밀폐 공간 작업에서 작업자를 추종하며, 작업자의 질식 상황을 실시간 확인 및 지휘소에 전송합니다. 내장된 통신 허브는 음영 지역에서도 안정적 연결을 보장하며, 필요 시 중계 기지국 역할을 수행합니다. SACR-GuardMate는 작업자 모니터링, 통신 및 긴급 지원을 통합하여 현장 안전 시스템을 반응형 프레임워크에서 사전 예방 프레임워크로 전환합니다.'
  },
  {
    id: 'GP055',
    hall: 'global',
  ...
  },
  {
    ...
    category: 'Fintech',
    product: '키포클라우드',
    description: 'Key4C는 클라우드 환경에서의 HSM(Hardware Security Module)과 KMS(Key Management System)을 SaaS 로 제공합니다. 뿐만아니라 이를 기반으로 아래의 기능도 SaaS 로 제공합니다. 1. 한 보안성 강화 OTP(One Time Password) 2. 간편로그인에 활용되는 JWT(Jason Web Token)의 보안 및 라이프 사이클 관리 3. K8S Resource에 대한 보안 서비스 4. 랜섬웨어 방어를 위한 CFS(Cryptography File System) 폴더 (for Linux, Win, MacOS)'
  },
  {
    id: 'GP059',
    hall: 'global',
  ...
  },
  {
    ...
    category: 'Smart Cities/Smart Home',
    product: '정수장 탁도측정기',
    description: '정수장 탁도측정기는 정수 처리 시설에서 물의 탁도를 0 NTU부터 1000 NTU까지 실시간으로 측정할 수 있는 장비입니다. 안정적인 탁도 측정을 통해 수질 변화를 신속하게 파악할 수 있으며, 안전하고 깨끗한 식수 공급을 위한 핵심 도구로 활용됩니다. 본 장비는 기존 장비로는 어려웠던 트라프 역세척 과정의 탁도에 대한 모니터링을 지원하도록 설계되어 운영 관리의 효율성을 높입니다. 또한 90도 산란광 방식을 적용해 국제 표준에 부합하는 정확한 측정 결과를 제공하며, 네트워크 연결을 통해 원격 모니터링과 데이터 관리가 가능해 운영자는 현장에 직접 가지 않고도 수질 상태를 확인하고 대응할 수 있습니다. 이 제품은 정수 처리 품질을 향상시키고 정수장 공정의 이상 및 수질 문제를 조기에 발견할 수 있게 합니다. 아울러, 역세척 과정의 최적화를 통한 에너지 절감, 에너지 효율을 향상시키고, 이에 따른 탄소 저감, 자동화 시스템을 통한 운영 효율성 강화를 지원합니다. 궁극적으로 안전한 식수 공급에 기여하며, 지속가능한 생활 환경 조성 및 정수장의 첨단화에도 기여합니다.'
  },
  {
    id: 'GP063',
    hall: 'global',
    ...
    category: 'Sustainability & Energy',
    product: '포엑스 스위트',
    description: 'FOEx Suite는 LNG, 수소, 암모니아 선박 등 폭발 위험 지역에서의 운전 안전, 설비 진단 및 국내외 방폭 관련 규정 준수를 위해, 웹 기반의 안전 진단, 관리 및 실시간 상태 진단 솔루션을 제공합니다. 이를 통해 다양한 정보를 통합 관리함으로써 운전 안전성과 시설 진단의 효율성을 높이고, 국제 규격 및 국내 규정(IECEx, KCs 등) 에 부합하는 폭발안전 관리가 가능합니다. 특히, FOEx Suite는 SaaS(Software-as-a-Service) 기반의 지능형 플랫폼으로서, 국제 방폭 표준인 IECEx 및 국내 KCs 방폭 인증의 검사 기준을 충족하며, 사용자의 플랜트 데이터베이스와 자산 관리 시스템을 함께 제공합니다. FOEx Suite는 시설의 데이터를 분석하여 이력을 평가하고, 이를 통해 위험도를 분석하고 안전한 시설운영이 될 수 있도록 지원합니다. 특히, 포엑스의 IoT 시스템과 연계하여 중요한 장비의 실시간 모니터링과 진단을 수행하며, AI 기반의 고장분석을 통해 보다 빠르고 정확한 진단 결과를 제공합니다.'
  },
  {
    id: 'GP064',
    hall: 'global',
  ...
  },
  {
    ...
    description: '뉴럴럭스(Neurélux)**는 뇌 건강을 위한 스마트 테라피 조명입니다. 810nm와 1064nm 이중 근적외선 파장을 활용해 뇌 혈류를 개선하고 세포의 미토콘드리아 활동을 지원하여 기억력, 집중력, 인지 기능 유지에 도움을 줍니다. 특허받은 반도체 방열 기술로 안정적인 파장과 안전성을 확보했으며, 일상적인 조명과 뇌 테라피를 하나로 결합해 집에서 간편하게 예방적 두뇌 관리가 가능합니다.'
  },
  {
    id: 'GP067',
    hall: 'global',
    ...
    category: 'Digital Health',
    product: '디지털 헬스케어 로봇',
    description: '. 종합 AI 건강 로봇 . 고출력 근적외선과 생성형 AI가 결합된 차세대 홈 헬스케어 로봇 . AI 건강 코칭음성 대화 기반'
  }
];

// 부스번호 정렬 함수
const sortByBoothNumber = (companies: Company[]): Company[] => {
  return [...companies].sort((a, b) => {
    const parseBoothNumber = (booth: string) => {
      const parts = booth.split('-');
      const prefix = parseInt(parts[0]) || 0;
      const suffix = parseInt(parts[1]) || 0;
      return prefix * 1000 + suffix;
    };
    return parseBoothNumber(a.boothNumber) - parseBoothNumber(b.boothNumber);
  });
};

// 부스번호 순으로 정렬된 데이터
export const eurekaCompaniesSorted: Company[] = sortByBoothNumber(eurekaCompanies);
export const globalCompaniesSorted: Company[] = sortByBoothNumber(globalCompanies);
...
export const getCompaniesByHall = (hall: 'eureka' | 'global'): Company[] => {
